...
def table():
    ...
    try:
        ...
        pass 
    try:# begin et end
        sql = "CREATE TABLE impacted_object (amended_arrival_time TIME, amended_departure_time TIME, departure_status VARCHAR(255), base_arrival_time TIME, base_departure_time TIME, arrival_status VARCHAR(255), cause VARCHAR(255), is_detour VARCHAR(255), lat FLOAT, lon FLOAT, id VARCHAR(255), label VARCHAR(255), name VARCHAR(255), stop_time_effect VARCHAR(255))"
        mycursor.execute(sql)
    except:
        pass
    try:# week_pattern
        ...
        pass
    try:
        sql = "CREATE TABLE stop_times (arrival_time TIME, departure_time TIME, drop_off_allowed VARCHAR(255), headsign INT, pickup_allowed VARCHAR(255), skipped_stop VARCHAR(255), lat FLOAT, lon FLOAT, id VARCHAR(255), label VARCHAR(255), name VARCHAR(255), utc_arrival_time TIME, utc_departure_time TIME)"
        mycursor.execute(sql)
    except:
        pass
