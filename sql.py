...
class Create():
    ...
    def table(self): # create sql table if not exist 
        ...
        try:
            sql = "CREATE TABLE disruptions (value INT, date DATE, begin TIME, end TIME, id VARCHAR(255), message TEXT, severity_effect VARCHAR(255), severity_name VARCHAR(255), trip_id VARCHAR(255) , trip_name INT)"
            self.mycursor.execute(sql)
        except:
            pass 
        try:        
            sql = "CREATE TABLE impacted_object (date DATE, begin TIME, end TIME, amended_arrival_time TIME, amended_departure_time TIME, arrival_status VARCHAR(255), base_arrival_time TIME, base_departure_time TIME, departure_status VARCHAR(255), cause VARCHAR(255), is_detour VARCHAR(255), lat FLOAT, lon FLOAT, id_impacted_stop VARCHAR(255), label_impacted_stop VARCHAR(255), name_impacted_stop VARCHAR(255), stop_time_effect VARCHAR(255))"
            self.mycursor.execute(sql)
        except:
            pass
        try:                                           
            sql = "CREATE TABLE vehicle_journeys (value INT, date DATE, begin DATE, end DATE, headsign VARCHAR(255), id VARCHAR(255), name VARCHAR(255), trip_id VARCHAR(255), trip_name VARCHAR(255))"
            self.mycursor.execute(sql)
        except:
            pass
        try:
            sql = "CREATE TABLE stop_times (date DATE, begin DATE, end DATE, arrival_time TIME, departure_time TIME, drop_off_allowed VARCHAR(255), headsign_stop VARCHAR(255), pickup_allowed VARCHAR(255), skipped_stop VARCHAR(255), lat FLOAT, lon FLOAT, id_stop_point VARCHAR(255), label_stop_point VARCHAR(255), name_stop_point VARCHAR(255), utc_arrival_time TIME, utc_departure_time TIME)"
            self.mycursor.execute(sql)
        except:
            pass
        try:                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
        ...
        for z in range(result):
            ...
            for x in data['disruptions']:
                ...
                for y in data['disruptions'][i]['impacted_objects']:
                    ...
                    sql = "INSERT INTO disruptions (value, date, begin, end, id, message, severity_effect, severity_name, trip_id, trip_name) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    ...
    def vehicle_journey(self): # insert vehicle_journey
        ...
        for z in range(result):
            ...
            for x in data['vehicle_journeys']:
                ...
                sql = "INSERT INTO vehicle_journeys (value, date, begin, end, headsign, id, name, trip_id, trip_name) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                ...
                sql = "INSERT INTO stop_times (date, begin, end, arrival_time, departure_time, drop_off_allowed, headsign_stop, pickup_allowed, skipped_stop, lat, lon, id_stop_point, label_stop_point, name_stop_point, utc_arrival_time, utc_departure_time) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
